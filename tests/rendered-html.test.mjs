...
test("server-renders an indexable, self-canonical home page", async () => {
  ...
  assert.match(html, /<h2[^>]*>Answers you can check for yourself\.<\/h2>/i);
  ...
  assert.match(html, /A short list of products worth understanding\./i);
...
test("publishes a source-linked USANA testing, quality, and manufacturing page", async () => {
  ...
  assert.match(html, /class="product-shelf-grid"/i);
  assert.match(html, /Four of the eight highlighted products, reviewed below\./i);
  ...
  assert.match(html, /<h2[^>]*>Decide where a routine starts\.<\/h2>/i);
  ...
  assert.match(
    html,
    /Nutrition guides for general readers\. We cite our sources\./i,
  );
  assert.equal(
    html.match(/<a[^>]*href="https:\/\/sissi\.usana\.com\/"/gi)?.length,
    2,
...
test("returns a useful custom 404", async () => {
  ...
  assert.equal(response.status, 404);
  const html = await response.text();
  assert.match(html, /This page wandered off/i);
