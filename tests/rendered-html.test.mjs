...
test("server-renders an indexable, self-canonical home page", async () => {
  ...
  assert.match(
    html,
    /<h1[^>]*>Nutrition guides and resources for your journey\.<\/h1>/i,
  );
  assert.match(
    html,
    /href="\/nutrition#guides-title"[^>]*>[\s\S]*Browse all 6 guides/i,
  );
  assert.match(
    html,
    /href="\/usana"[^>]*>USANA<\/a>/i,
  );
  assert.match(html, /Featured partner · USANA/i);
  assert.match(html, /Superb quality you can inspect\. Innovation you can trace\./i);
...
test("publishes a source-linked USANA quality and innovation page", async () => {
  ...
  assert.match(
    html,
    /<meta name="description" content="A source-linked look at USANA product quality, manufacturing, third-party listings, and product innovation, with the limits of each signal kept clear\."/i,
  );
  assert.match(
    html,
  ...
  assert.match(html, /Reviewed August 29, 2026/i);
  ...
  assert.equal(
    html.match(/<a[^>]*href="https:\/\/sissi\.usana\.com\/"/gi)?.length,
    1,
    "expected exactly one storefront link on the USANA page",
  );
  assert.match(
    html,
...
test("publishes a nutrition hub with only complete guides linked", async () => {
  ...
  assert.match(html, /Six practical places to begin/i);
...
test("serves the full-resolution hero image referenced in rendered HTML", async () => {
  const homeResponse = await render("/");
  const html = await homeResponse.text();
  assert.match(
    html,
    /<img[^>]*src="\/images\/joy-health-morning\.webp"[^>]*width="1672"[^>]*height="941"/i,
  );
  assert.doesNotMatch(html, /\/_next\/image\?[^"']*joy-health-morning/i);

  const response = await worker.fetch(
    new Request(
      "https://joyhealth.cc/images/joy-health-morning.webp",
