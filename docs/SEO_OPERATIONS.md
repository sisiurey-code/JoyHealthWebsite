...
compresses a response, and the Worker compares tags weakly. On 2026-09-24 it
printed `200`: Cloudflare removed the ETag from HTML responses (Markdown,
text, and data files kept theirs). Cloudflare drops ETags on HTML when Email
Obfuscation or Automatic HTTPS Rewrites is on. The site publishes no email
addresses and every link is already HTTPS, so turning both off in the zone
settings costs nothing and restores HTML revalidation.
